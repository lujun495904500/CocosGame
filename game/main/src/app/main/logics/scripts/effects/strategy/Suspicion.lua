...
local Suspicion = class("Suspicion", import("._StrategyBase"))

-- 构造函数
function Suspicion:ctor(config)
	if config then
...
-- 战场上
function Suspicion:executeInBattle(onComplete)
						...
						if ishit then
							target:getEntity():addBuff(bid,{
								...
								msgwin:appendMessage({
									texts = gameMgr:getStrings("SKILL_SUSPICION",{
...
-- 地图上
function Suspicion:executeInMap(onComplete)
