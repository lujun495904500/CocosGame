...
local AvoidAttack = class("AvoidAttack", import("._StrategyBase"))

-- 构造函数
function AvoidAttack:ctor(config)
	if config then
...
-- 战场上
function AvoidAttack:executeInBattle(onComplete)
				...
				if ishit then
					self.role:getTeam():getEnemyTeam():getEntity():addBuff(bid,{
						...
						msgwin:appendMessage({
							texts = gameMgr:getStrings("SKILL_AVOIDATTACK"),
...
-- 地图上
function AvoidAttack:executeInMap(onComplete)
