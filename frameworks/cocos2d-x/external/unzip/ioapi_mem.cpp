...
namespace cocos2d {
    ...
    voidpf ZCALLBACK fopen_mem_func64 (voidpf opaque,
                                          const void* filename,
                                          int mode)
    {
        ourmemory_t *mem = (ourmemory_t*)malloc(sizeof(ourmemory_t));
    ...
    long ZCALLBACK ftell_mem_func (voidpf opaque, voidpf stream)
    {
        ...
        return (long)mem->cur_offset;
    }
	ZPOS64_T ZCALLBACK ftell_mem_func64(voidpf opaque, voidpf stream)
	{
		ourmemory_t *mem = (ourmemory_t *)stream;
	...
	long ZCALLBACK fseek_mem_func64(voidpf opaque, voidpf stream, ZPOS64_T offset, int origin)
	{
		ourmemory_t *mem = (ourmemory_t *)stream;
		ZPOS64_T new_pos;
		switch (origin) {
    ...
    {
		pzlib_filefunc_def->zopen64_file = fopen_mem_func64;
		pzlib_filefunc_def->zread_file = fread_mem_func;
		pzlib_filefunc_def->zwrite_file = fwrite_mem_func;
		pzlib_filefunc_def->ztell64_file = ftell_mem_func64;
		pzlib_filefunc_def->zseek64_file = fseek_mem_func64;
		pzlib_filefunc_def->zclose_file = fclose_mem_func;
		pzlib_filefunc_def->zerror_file = ferror_mem_func;
		pzlib_filefunc_def->opaque = NULL;
    }
